...
type NormalizedSpec = { field:string; label:string; rawField:string; rawValue:string; value:string|number|boolean|string[]; unit:string|null; method:string; confidence:SpecConfidence; sourceUrl:string; collectedAt:string }
type ManufacturerSpecRecord = { candidateId:string; category:Category; brand:string; officialUrl:string; collectedAt:string; coverage:{rawFields:number;normalized:number}; specs:NormalizedSpec[]; missingFields:string[]; review:{status:'pending';reason:string} }
type ManufacturerSpecDocument = { summary:{eligibleCandidates:number;collected:number;remaining:number;failures:number;rawValues:number;normalizedValues:number;byCategory:Record<string,{records:number;normalizedValues:number}>;byConfidence:Record<SpecConfidence,number>}; records:ManufacturerSpecRecord[] }
const confidenceLabels:Record<SpecConfidence,string>={high:'Confiance élevée',medium:'Confiance moyenne',low:'Confiance faible'}
const confidenceTones:Record<SpecConfidence,string>={high:'green',medium:'yellow',low:'neutral'}
const extractionLabels:Record<string,string>={'json-ld':'JSON-LD','spec-table':'Tableau constructeur','definition-list':'Liste de définitions','spec-list':'Liste étiquetée','meta':'Métadonnées de page'}
const formatSpecValue=(spec:NormalizedSpec)=>Array.isArray(spec.value)?spec.value.join(', '):typeof spec.value==='boolean'?(spec.value?'Oui':'Non'):`${spec.value}${spec.unit?` ${spec.unit}`:''}`
...
function CatalogBots(){
  ...
  const specsById=useMemo(()=>new Map((manufacturerSpecs?.records??[]).map(item=>[item.candidateId,item])),[manufacturerSpecs])
  ...
  return <main className="shell app-page bots-page">
    ...
    <div className="bots-layout"><section className="bot-panel"><div className="bot-heading"><span><Bot/></span><div><Pill tone="green">Automatisation gratuite</Pill><h2>Bot de découverte multimarque</h2><p>{automaticCandidates.length} candidats produits, {hardwareIdentifiers.length} identifiants matériels et {automaticRejections.length} faux positif restent en quarantaine. {coverageReport?.totals.officialCandidates??0} candidats viennent d’index officiels ; {manufacturerEvidence?.summary.collected??0} ont des métadonnées constructeur collectées, {manufacturerVerified} sont validés humainement et {integratedCount} sont intégrés. {manufacturerSpecs?.summary.collected??0} fiches ont des caractéristiques normalisées à relire.</p></div></div>
      ...
      {manufacturerSpecs&&<section className="spec-panel" aria-label="Caractéristiques normalisées"><div className="coverage-heading"><div><small>CARACTÉRISTIQUES NORMALISÉES</small><b>{manufacturerSpecs.summary.collected} fiches · {manufacturerSpecs.summary.normalizedValues} valeurs</b></div><span>{manufacturerSpecs.summary.rawValues} valeurs brutes relevées · {manufacturerSpecs.summary.remaining} fiches restantes</span></div>
        <div className="spec-confidence">{(['high','medium','low'] as SpecConfidence[]).map(level=><span key={level} className={level}><b>{manufacturerSpecs.summary.byConfidence[level]}</b>{confidenceLabels[level]}</span>)}{Object.entries(manufacturerSpecs.summary.byCategory).map(([item,count])=><span key={item}><b>{count.normalizedValues}</b>{categoryLabels[item as Category]}</span>)}</div>
        <p>Chaque valeur conserve la valeur brute publiée, le champ d’origine, la source constructeur, la date de collecte, la méthode d’extraction et un niveau de confiance. Une caractéristique absente de la fiche reste inconnue : elle n’est jamais transformée en réponse négative, et aucune valeur normalisée ne déclenche de promotion.</p></section>}
