...
type DiscoveryCoverage = { category:Category; registeredBrands:string[]; observedBrands:string[]; candidates:number; promotable:number; hardwareIdentifiers:number; rejected:number }
type DiscoveryReport = { generatedAt:string; collection:{date:string|null;status:'not-run'|'success'|'partial'|'failed';queriesAttempted:number;queriesCompleted:number;pagesRequested:number;requestsSucceeded:number;budgetExhausted:boolean;errors:string[]}; totals:{registeredQueries:number;registeredBrands:number;registeredCategories:number;categoriesWithCandidates:number;candidates:number;hardwareIdentifiers:number;rejected:number}; coverage:DiscoveryCoverage[] }
const registryConfig=sourceRegistry as {policy:{minimumDelayMs:number;wikidataPageSize?:number;maxLagSeconds?:number};queries:{category:Category;brand:string;query:string}[]}
...
function CatalogBots(){
  ...
  const [automaticCandidates,setAutomaticCandidates]=useState<BotCandidate[]>([]),[hardwareIdentifiers,setHardwareIdentifiers]=useState<BotCandidate[]>([]),[automaticRejections,setAutomaticRejections]=useState<BotCandidate[]>([]),[verifications,setVerifications]=useState<CandidateVerification[]>([]),[coverageReport,setCoverageReport]=useState<DiscoveryReport|null>(null)
  useEffect(()=>{let active=true;Promise.all([import('./catalog/discovery.generated.json'),import('./catalog/hardware-identifiers.generated.json'),import('./catalog/rejected.generated.json'),import('./catalog/candidate-verification.generated.json'),import('./catalog/discovery-report.generated.json')]).then(([discovery,hardware,rejected,verification,report])=>{if(!active)return;setAutomaticCandidates((discovery.default as unknown as {candidates:BotCandidate[]}).candidates);setHardwareIdentifiers((hardware.default as unknown as {identifiers:BotCandidate[]}).identifiers);setAutomaticRejections((rejected.default as unknown as {candidates:BotCandidate[]}).candidates);setVerifications((verification.default as unknown as {candidates:CandidateVerification[]}).candidates);setCoverageReport(report.default as unknown as DiscoveryReport)});return()=>{active=false}},[])
  ...
  return <main className="shell app-page bots-page">
    ...
    <div className="bots-layout"><section className="bot-panel"><div className="bot-heading"><span><Bot/></span><div><Pill tone="green">Automatisation gratuite</Pill><h2>Bot de découverte multimarque</h2><p>{automaticCandidates.length} candidats produits, {hardwareIdentifiers.length} identifiants matériels et {automaticRejections.length} faux positif restent en quarantaine. {manufacturerVerified} références ont une preuve constructeur et {integratedCount} sont intégrées au catalogue.</p></div></div>
      ...
      {coverageReport&&<section className="coverage-panel" aria-label="Couverture du bot"><div className="coverage-heading"><div><small>COUVERTURE MESURÉE</small><b>{coverageReport.totals.registeredBrands} marques · {coverageReport.totals.registeredQueries} recherches</b></div><span>{coverageReport.totals.categoriesWithCandidates}/{coverageReport.totals.registeredCategories} catégories observées</span></div><div className="coverage-grid">{coverageReport.coverage.map(item=><article key={item.category} className={item.candidates+item.hardwareIdentifiers?'covered':''}><b>{categoryLabels[item.category]}</b><strong>{item.registeredBrands.length}</strong><small>marques suivies</small><span>{item.candidates+item.hardwareIdentifiers} résultat{item.candidates+item.hardwareIdentifiers>1?'s':''}</span></article>)}</div><p>{coverageReport.collection.status==='not-run'?'Le prochain passage hebdomadaire remplira les nouvelles catégories.':`Dernier passage : ${coverageReport.collection.queriesCompleted}/${coverageReport.collection.queriesAttempted} recherches terminées le ${coverageReport.collection.date}.`} Chaque résultat reste en quarantaine.</p></section>}
